...
| `@slider` | 幻灯片：自动轮播，循环切换，hover 暂停（实用） |
| `@ticket` | 票根：撕边虚线 + 半圆缺口，票根造型（克制） |
| `@panorama` | 全景横幅：超宽横向滚动拼接，沉浸式浏览（实用） |
| `@circle` | 圆形相册：图片裁切为圆形，居中错落排布（美观） |
| `@diamond` | 菱形宝石：图片裁切为菱形，错位排列（艺术） |
| `@wave` | 波浪：图片沿正弦曲线高低错落，流动韵律感（艺术） |
...
自动轮播，每 4 秒切换下一张，循环播放，鼠标悬停时暂停。暗色底衬托，左右渐变遮罩暗示可滑动，底部指示点显示当前位置。实用：故事线、步骤展示、产品图集。
...
超宽横向滚动，图片近似无缝拼接，沉浸式浏览。暗色底 + scroll-snap 自动对齐，左右滑动浏览。实用：风景、全景、长卷展示。
...
图片裁切为圆形，白色描边，居中错落排布（奇偶张上下错位）。hover 时拉直放大。美观：头像、徽章、人物展示。
...
图片用 clip-path 裁切为菱形，奇数张下移形成交错蜂巢感，drop-shadow 投影模拟立体感。hover 时放大提亮。艺术：装饰性展示。
...
图片沿正弦曲线高低错落排列（4 张一个周期：高→次高→次低→低），营造流动韵律感。hover 时拉平放大上浮。艺术：节奏感展示。
