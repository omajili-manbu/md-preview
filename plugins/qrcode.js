...
QRCodeModel.prototype={
	...
	isDark:function(row,col){if(row<0||this.moduleCount<=row||col<0||this.moduleCount<=col){throw new Error(row+","+col);}
...
QRCodeModel.createBytes=function(buffer,rsBlocks){
	...
	for(var r=0;r<rsBlocks.length;r++){var dcCount=rsBlocks[r].dataCount;var ecCount=rsBlocks[r].totalCount-dcCount;
...
var QRUtil={
	...
	getBCHTypeInfo:function(data){var d=data<<10;while(QRUtil.getBCHDigit(d)-QRUtil.getBCHDigit(QRUtil.G15)>=0){d^=(QRUtil.G15<<(QRUtil.getBCHDigit(d)-QRUtil.getBCHDigit(QRUtil.G15)));}
...
QRRSBlock.RS_BLOCK_TABLE=[
	...
	[5,109,87,1,110,88],[5,65,41,5,66,42],[5,54,24,7,55,25],[11,36,12,5,37,13],
...
QRRSBlock.getRSBlocks=function(typeNumber,errorCorrectLevel){
	...
	var length=rsBlock.length/3;var list=[];for(var i=0;i<length;i++){var count=rsBlock[i*3+0];var totalCount=rsBlock[i*3+1];var dataCount=rsBlock[i*3+2];for(var j=0;j<count;j++){list.push(new QRRSBlock(totalCount,dataCount));}}return list;};
...
function drawQRCode(canvas, text, size) {
	...
	var nWidth = size / nCount;
	var nHeight = size / nCount;
