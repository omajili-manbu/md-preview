...
2. **命令缩写**：VRP 允许 `int` 代替 `interface`、`ospf 1` 写成 `osp` 等，但 PKT 只认全称，缩写会漏识别。
...
5. **顺序问题**：PKT **强制按固定顺序**输出步骤（VLAN → 接口 → IS-IS → OSPF → ...），与原始配置在文件中的先后顺序可能不一致，依赖顺序的命令（如先 `ip route-static` 再 `route-policy` 引用）在视觉上可能显得"跳"。
